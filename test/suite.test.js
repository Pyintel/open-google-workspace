...
async function runGoogleWorkspaceTestSuite() {
  ...
  // Test 1: auth_login (Explicit First-Time User Login Prompt)
  totalTested++;
  ...
  assert.strictEqual(resAuth1.account, "user@example.com");
  assert.ok(resAuth1.authUrl.includes("accounts.google.com"));
  passed++;
  console.log("  ✅ Tool 1/38: auth_login (Explicit First-Time User Login Prompt) PASSED");

  // Test 2: auth_login (Second Account)
  totalTested++;
  ...
  assert.strictEqual(resAuth2.account, "work@company.com");
  passed++;
  console.log("  ✅ Tool 2/38: auth_login (Second Account Registration) PASSED");
  ...
  totalTested++;
  const resTaskComp = JSON.parse(await gwsTools.tasks_complete.execute({ account: "user@example.com", taskId: resTaskCreate.taskId }));
  ...
  totalTested++;
  const resCalUpdate = JSON.parse(await gwsTools.calendar_update.execute({ account: "user@example.com", eventId: resCalCreate.eventId, summary: "Pyintel Core Sync (Rescheduled)" }));
  ...
  totalTested++;
  const resCalDel = JSON.parse(await gwsTools.calendar_delete.execute({ account: "user@example.com", eventId: resCalCreate.eventId }));
  ...
  // Tool 26: docs_get
  totalTested++;
  const resDocGet = JSON.parse(await gwsTools.docs_get.execute({ account: "user@example.com", documentId: resDocCreate.documentId }));
  ...
  totalTested++;
  const resDocApp = JSON.parse(await gwsTools.docs_append_text.execute({ account: "user@example.com", documentId: resDocCreate.documentId, text: "\nAdditional section appended." }));
